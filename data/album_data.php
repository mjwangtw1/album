<?php

$data = array(
	'site_title'=> 'Project Photo w/Bootstrap+Flickr Dynamic',
	'nav_bar_title'=>'Mou Photos 2015/3/2',
	'welcome_title'=>'Yo Dawg!',
	'welcome_message'=>'Trial album w/BT3+Flickr',
	'image_thumb_array'=> array(
	array(
			'img_url'=>'https://farm4.staticflickr.com/3755/10565638896_530c0595c5_s.jpg',
			'img_alt'=>'Bar@Pittsburgh',
			 'tag' => 'pittsburgh'
			),
		array(
			'img_url'=>'https://farm8.staticflickr.com/7323/10565637196_2296f583fd_s.jpg',
			'img_alt'=>'Famous Primantis sandwich',
			 'tag' => 'pittsburgh'
			),
		array(
			'img_url'=>'https://farm3.staticflickr.com/2851/10565986203_8255a93880_s.jpg',
			'img_alt'=>'Robber duck and ducklings',
			 'tag' => 'pittsburgh'
			),
		array(
			'img_url'=>'https://farm8.staticflickr.com/7323/10565637196_2296f583fd_s.jpg',
			'img_alt'=>'Famous Primantis sandwich',
			 'tag' => 'pittsburgh'
			),
		array(
			'img_url'=>'https://farm8.staticflickr.com/7323/10565637196_2296f583fd_s.jpg',
			'img_alt'=>'Famous Primantis sandwich',
			 'tag' => 'pittsburgh'
			),
		array(
			'img_url'=>'https://farm8.staticflickr.com/7323/10565637196_2296f583fd_s.jpg',
			'img_alt'=>'Famous Primantis sandwich',
			 'tag' => 'pittsburgh'
			),
		array(
			'img_url'=>'https://farm8.staticflickr.com/7323/10565637196_2296f583fd_s.jpg',
			'img_alt'=>'Famous Primantis sandwich',
			 'tag' => 'pittsburgh'
			),
		array(
			'img_url'=>'https://farm8.staticflickr.com/7323/10565637196_2296f583fd_s.jpg',
			'img_alt'=>'Famous Primantis sandwich',
			 'tag' => 'pittsburgh'
			),
		array(
			'img_url'=>'https://farm8.staticflickr.com/7323/10565637196_2296f583fd_s.jpg',
			'img_alt'=>'Famous Primantis sandwich',
			 'tag' => 'pittsburgh'
			),
		array(
			'img_url'=>'https://farm8.staticflickr.com/7323/10565637196_2296f583fd_s.jpg',
			'img_alt'=>'Famous Primantis sandwich',
			 'tag' => 'pittsburgh'
			),
		array(
			'img_url'=>'https://farm8.staticflickr.com/7323/10565637196_2296f583fd_s.jpg',
			'img_alt'=>'Famous Primantis sandwich',
			 'tag' => 'pittsburgh'
			),
		array(
			'img_url'=>'https://farm8.staticflickr.com/7323/10565637196_2296f583fd_s.jpg',
			'img_alt'=>'Famous Primantis sandwich',
			 'tag' => 'pittsburgh'
			),
		array(
			'img_url'=>'https://farm8.staticflickr.com/7323/10565637196_2296f583fd_s.jpg',
			'img_alt'=>'Famous Primantis sandwich',
			 'tag' => 'pittsburgh'
			),
		array(
			'img_url'=>'https://farm6.staticflickr.com/5536/10565542735_4a91b159e4_s.jpg',
			'img_alt'=>'Checking stuff',
			 'tag' => 'pittsburgh'
			),
	),
	'nav_icons' => array(
			array(
			'name' =>'Pitt Life',
			'id'   =>'cat_pitt_life',
			'cat'  =>'cat',
			'desc' =>'Life in Pittsburgh is somewhat bittersweet. This album contains some cool Pittsburgh Moments'
			),
			array(
			'name' =>'Rand Stuff',
			'id'   =>'cat_rand_stuff',
			'cat'  =>'cat',
			'desc' =>'Just some random stuffs around'
			),
			array(
			'name' =>'Fun Stuff',
			'id'   =>'cat_fun_stuff',
			'cat'  =>'cat',
			'desc' =>'some wtf fun stuff'
			),
			array(
			'name' =>'Work Time',
			'id'   =>'cat_work_time',
			'cat'  =>'cat',
			'desc' =>'moments at work'
			),
			array(
			'name' =>'NYC',
			'id'   =>'cat_nyc',
			'cat'  =>'cat',
			'desc' =>'NYC the big apple. Glarious and Sexy'
			),
			array(
			'name' =>'Travel',
			'id'   =>'cat_travel',
			'cat'  =>'cat',
			'desc' =>'Some of the best moments of travels'
			),

		),

);

?>